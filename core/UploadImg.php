...
class UploadImg 
{
    ...
    public static function saveFile($transaction_id, $instance)
    {
        ...
        if (empty($_FILES['upload_file']['name'])) {
            ...
            $fileName = $transaction_id . '-' . uniqid() . '.' . $file_ext;
    ...
    public static function saveTicketFile($transaction_id, $instance)
    {
        $uploadDir = 'public/Ticketing/screenshots/';
    ...
    public static function saveCommentFile($ticketId, $instance)
    {
        $uploadDir = 'public/ticket/comment/';
        ...
        if (empty($_FILES['upload_file']['name'])) {
            ...
            // Validate file type
            if (! in_array($file_ext, ['jpeg', 'jpg', 'png'])) {
                $instance->error('upload_file', 'File type not supported. Please choose jpg or png file.')
                    ->throw();
        }

            // Sanitize filename and create a unique name
            $fileName = $ticketId . '-' . uniqid() . '.' . $file_ext;
            $upload_path = $uploadDir . $fileName;

            // Check for successful upload
                if (! move_uploaded_file($_FILES['upload_file']['tmp_name'], $upload_path)) {
                    $instance->error('upload_file', 'Failed to move uploaded file.')
                        ->throw();
                }
