                    ...
                    <ul class="list-group list-group-flush">
                        <?php foreach($monthBalance as $balance):?>
                            <li class="list-group-item fw-bold">
                                ...
                                </span> <?= format_Number($balance['total_amount']) ?>
                            </li>
                        <?php endforeach;?>
                    </ul>
                </div>
                <div class="summary-card">
                    ...
                    <ul class="list-group list-group-flush">
                        <?php foreach ($transactions as $transaction):?>
                            <li class="list-group-item"><?= $transaction['trxDate']?> - GMD <?= format_Number($transaction['debit_amount']) ?> </li>
                        <?php endforeach;?>
                    </ul>
                </div>
            </div>
                    ...
                    <div class="tab-content mt-3">   
                        ...
                        <div id="addMoney" class="tab-pane fade show active">
                            ...
                            <form action="<?= route('instruct/new/add') ?>" method="post" enctype="multipart/form-data">
                                ...
                                <div class="mb-3">
                                    <label for="Transaction_amount" class="form-label">Agent Name</label>
                                    <input type="text" disabled class="form-control" min="0" id="Transaction_amount" value="<?= ($transaction['agent_name']) ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="Transaction_amount" class="form-label">Agent Wallet Number</label>
                                    <input type="text" disabled class="form-control" min="0" id="Transaction_amount" value="<?= ($transaction['agent_wallet_number']) ?>">
                                </div>
                                <?php endforeach;?>
                                ...
                                <h4>Agent's Bank Details (PAYEE details)</h4>
                                <hr>
                                    <div class="mb-3">
                                    ...
                                    <hr>
                                    <h4>APS Bank Details (PAYER details)</h4>
                                    <hr>
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select <?php if(isset($errors['agent_paid_bank_name'])):?> is-invalid <?php endif;?>" id="agent_paid_bank_name" name="agent_paid_bank_name">
                                            <option value="">Choose APS Bank Name</option>
                                            <?php foreach($bankList as $bank):?>
                                            <option value="<?= $bank['bank_name'] ?>"><?= $bank['bank_name'] ?></option>
                                            <?php endforeach;?>
                                        </select>
