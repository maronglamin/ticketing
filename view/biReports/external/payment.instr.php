...
<?php if ($_SERVER['REQUEST_URI'] !== root(). '/note/search'):?>
<div class="flex flex-col md:flex-row bg-white rounded-lg p-6 border border-gray-300 shadow-md">
    ...
    <div class="flex items-center justify-between mb-4">
            ...
            <h3 id="reportTitle" class="text-lg font-semibold text-black">Pay Instruction Report</h3>

            <!-- Right Side: Inputs & Button -->
            <form action="<?= route('note/search')?>" method="post" class="flex items-center space-x-4">
                <!-- Start Time -->
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-max border border-gray-200">
                <thead class="bg-gray-100">
                    <tr>
                        <th class="border px-4 py-2">Form id</th>
                        <th class="border px-4 py-2">KillMoney_FormId</th>
                        <th class="border px-4 py-2">WithdrawTo TrustTxnId</th>
                        <th class="border px-4 py-2">Date</th>
                        <th class="border px-4 py-2">Agent Name</th>
                        <th class="border px-4 py-2">AgentBank Acc_name</th>
                        <th class="border px-4 py-2">AgentBank Acc_num</th>
                        <th class="border px-4 py-2">transaction_filename</th>
                        <th class="border px-4 py-2">Transaction Amount</th>
                        <th class="border px-4 py-2">Status</th>
                        <th class="border px-4 py-2">SignedOne by</th>
                        <th class="border px-4 py-2">SignedOne at</th>
                        <th class="border px-4 py-2">SignedTwo by</th>
                        <th class="border px-4 py-2">SignedTwo at</th>
                        <th class="border px-4 py-2">Instruction Closed_by</th>
                        <th class="border px-4 py-2">Instruction Closed_at</th>
                    </tr>
                </thead>
                <tbody>
                <?php foreach($results as $value):?>
                    <tr>
                        <td class="px-4 py-2 text-start"><?= $value['Form_id']?></td>
                        <td class="px-4 py-2 text-start"><?= $value['KillMoney_FormId']?></td>
                        <td class="px-4 py-2 text-start"><?= $value['WithdrawTo_TrustTxnId']?></td>
                        <td class="px-4 py-2 text-end"><?= regularDate($value['created_at'])?></td>
                        <td class="px-4 py-2 text-start"><?= ($value['agent_name'])?></td>
                        <td class="px-4 py-2 text-start"><?= $value['agent_bank_acc_name']?></td>
                        <td class="px-4 py-2 text-start"><?= $value['agent_bank_acc_num']?></td>
                        <td class="px-4 py-2 text-end"><?= $value['transaction_filename']?></td>
                        <td class="px-4 py-2 text-strat"><?= format_number($value['Transaction_amount'])?></td>
                        <td class="px-4 py-2 text-start"><?= $value['transaction_status']?></td>
                        <td class="px-4 py-2 text-end"><?= $value['sign_1']?></td>
                        <td class="px-4 py-2 text-end"><?= regularDateTime($value['sign_at_1'])?></td>
                        <td class="px-4 py-2 text-end"><?= ($value['sign_2'])?></td>
                        <td class="px-4 py-2 text-end"><?= regularDateTime($value['sign_at_2'])?></td>
                        <td class="px-4 py-2 text-end"><?= ($value['closed_by'])?></td>
                        <td class="px-4 py-2 text-end"><?= regularDateTime($value['closed_at'])?></td>
                    </tr>
                <?php endforeach;?>
                </tbody>
            </table>
